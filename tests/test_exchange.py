...
def test_which_returns_methods(patch_agentmail, tmp_db):
    xh.process_email(_payload("WHICH"), db_path=tmp_db)
...
def test_offer_valid_creates_pending(patch_agentmail, tmp_db):
    xh.process_email(_payload("OFFER | $2 for SOL", text=_valid_offer(200)), db_path=tmp_db)
    replies = patch_agentmail._replies
    assert len(replies) == 1
    ...
    # Check DB
    pending = xdb.get_pending(tmp_db)
...
def test_offer_amount_too_high(patch_agentmail, tmp_db):
    xh.process_email(_payload("OFFER | $10", text=_valid_offer(1000)), db_path=tmp_db)
    replies = patch_agentmail._replies
    assert len(replies) == 1
    ...
    # No DB entry
    assert len(xdb.get_pending(tmp_db)) == 0


# --- OFFER too low ---

def test_offer_amount_too_low(patch_agentmail, tmp_db):
    xh.process_email(_payload("OFFER | $0.50", text=_valid_offer(50)), db_path=tmp_db)
    replies = patch_agentmail._replies
    assert len(replies) == 1
    assert replies[0]["headers"]["X-Envelopay-Type"] == "OOPS"
    body = _extract_json(replies[0]["text"])
...
def test_unknown_type_returns_oops(patch_agentmail, tmp_db):
    xh.process_email(_payload("HELLO"), db_path=tmp_db)
    replies = patch_agentmail._replies
    assert len(replies) == 1
    assert replies[0]["headers"]["X-Envelopay-Type"] == "OOPS"
    body = _extract_json(replies[0]["text"])
...
def test_skips_own_messages(patch_agentmail, tmp_db):
    xh.process_email(_payload("WHICH", from_addr=xc.EXCHANGE_INBOX), db_path=tmp_db)
    assert len(patch_agentmail._replies) == 0


# --- Approve transaction ---

def test_approve_sends_sol_and_accept(patch_agentmail, tmp_db):
    # Create a pending transaction
    xh.process_email(_payload("OFFER | $2", text=_valid_offer(200)), db_path=tmp_db)
    patch_agentmail._replies.clear()

    tx = xdb.get_pending(tmp_db)[0]
    ...
    # Mark approved in DB
    xdb.approve_transaction(tmp_db, tx["id"], "fake_sol_tx_hash")
    ...
    # Check DB
    updated = xdb.get_transaction(tmp_db, tx["id"])
...
def test_reject_sends_oops(patch_agentmail, tmp_db):
    # Create a pending transaction
    xh.process_email(_payload("OFFER | $3", text=_valid_offer(300)), db_path=tmp_db)
    patch_agentmail._replies.clear()

    tx = xdb.get_pending(tmp_db)[0]

    # Reject
    xdb.reject_transaction(tmp_db, tx["id"])
    with patch("exchange.handler.AgentMail", return_value=patch_agentmail), \
         patch.object(xh, "_get_last_message_info", return_value=("mock-msg-id", "alice@test.com")):
...
def test_stats_empty(tmp_db):
    stats = xdb.get_stats(tmp_db)
    assert stats["count"] == 0
    assert stats["total_cents"] == 0


def test_stats_with_approved(tmp_db):
    tx_id = xdb.create_transaction(
        db_path=tmp_db, email_from="a@b.com",
    ...
    xdb.approve_transaction(tmp_db, tx_id, "tx_hash")
    stats = xdb.get_stats(tmp_db)
...
def test_offer_missing_wallet(patch_agentmail, tmp_db):
    body = json.dumps({
        "v": "0.1.0", "type": "offer",
    ...
    xh.process_email(_payload("OFFER | $2", text=body), db_path=tmp_db)
    replies = patch_agentmail._replies
    assert len(replies) == 1
    assert replies[0]["headers"]["X-Envelopay-Type"] == "OOPS"
...
def test_non_protocol_email_ignored(patch_agentmail, tmp_db):
    xh.process_email(_payload("hello there", text="just chatting"), db_path=tmp_db)
    assert len(patch_agentmail._replies) == 0
...
def test_duplicate_offer_same_message_id_skipped(patch_agentmail, tmp_db):
    ...
    xh.process_email(p1, db_path=tmp_db)
    assert len(patch_agentmail._replies) == 1  # first gets a reply

    patch_agentmail._replies.clear()
    xh.process_email(p2, db_path=tmp_db)
...
def test_approve_already_approved_fails(tmp_db):
    ...
    assert xdb.approve_transaction(tmp_db, tx_id, "tx_hash_1") is True

    # Second approve fails (already approved, not pending)
    assert xdb.approve_transaction(tmp_db, tx_id, "tx_hash_2") is False

    # Status is still the first approval
    tx = xdb.get_transaction(tmp_db, tx_id)
    assert tx["sol_tx"] == "tx_hash_1"


# --- Bug 4: OFFER when rate API fails returns OOPS ---

def test_offer_rate_failure_returns_oops(patch_agentmail, tmp_db):
...
def test_which_rate_failure_returns_oops(patch_agentmail, tmp_db):
...
def test_invalid_base58_wallet_returns_oops(patch_agentmail, tmp_db):
    """Wallet with invalid base58 characters should be rejected."""
    # 'O' (capital O) and '0' (zero) are not in base58, but '0' is not in base58
    ...
    # The wallet is 44 chars, but has bad base58 chars
    xh.process_email(_payload("OFFER | $2", text=offer), db_path=tmp_db)
    replies = patch_agentmail._replies
    assert len(replies) == 1
    assert replies[0]["headers"]["X-Envelopay-Type"] == "OOPS"
    body = _extract_json(replies[0]["text"])
...
def test_multiline_json_parsing(patch_agentmail, tmp_db):
...
def _create_pending_offer(db_path: str, amount_cents: int = 200,
...
def test_cashapp_notification_auto_approves(patch_agentmail, tmp_db):
    """Forwarded CashApp notification matches pending OFFER and auto-approves."""
    tx_id = _create_pending_offer(tmp_db, amount_cents=200, message_id="offer-msg-1")

    with patch.object(xh, "send_sol", return_value="fake_sol_tx_abc") as mock_sol, \
         patch.object(xh, "send_accept") as mock_accept:
        xh.process_email(_cashapp_notification("$2.00"), db_path=tmp_db)
...
def test_venmo_notification_auto_approves(patch_agentmail, tmp_db):
    ...
    with patch.object(xh, "send_sol", return_value="fake_sol_tx_venmo") as mock_sol, \
         patch.object(xh, "send_accept") as mock_accept:
        xh.process_email(_venmo_notification("$5.00"), db_path=tmp_db)
...
def test_payment_notification_no_match_ignored(patch_agentmail, tmp_db):
...
def test_payment_notification_wrong_amount_no_match(patch_agentmail, tmp_db):
    """Payment notification with wrong amount doesn't match any pending OFFER."""
    _create_pending_offer(tmp_db, amount_cents=200, message_id="offer-msg-3")

    with patch.object(xh, "send_sol") as mock_sol, \
         patch.object(xh, "send_accept") as mock_accept:
        # Notification is $3.00 but pending OFFER is $2.00
        xh.process_email(_cashapp_notification("$3.00"), db_path=tmp_db)
...
def test_payment_notification_fifo_oldest_matched(patch_agentmail, tmp_db):
    """Multiple pending OFFERs: oldest matching amount gets filled (FIFO)."""
    tx_id_1 = _create_pending_offer(tmp_db, amount_cents=200, thread_id="thread-old",
                                     message_id="offer-old")
    tx_id_2 = _create_pending_offer(tmp_db, amount_cents=200, thread_id="thread-new",
    ...
    assert xdb.get_transaction(tmp_db, tx_id_1)["status"] == "approved"
    assert xdb.get_transaction(tmp_db, tx_id_2)["status"] == "pending"
...
def test_claim_prevents_double_pay(patch_agentmail, tmp_db):
    """Two concurrent payment notifications for the same OFFER: only one sends SOL."""
    tx_id = _create_pending_offer(tmp_db, amount_cents=200, message_id="offer-race-1")
    ...
    with patch.object(xh, "send_sol", side_effect=tracking_send_sol), \
        ...
        xh.process_email(notif1, db_path=tmp_db)
        xh.process_email(notif2, db_path=tmp_db)
...
def test_accept_goes_to_original_sender(patch_agentmail, tmp_db):
    """send_accept uses to_addr parameter, not thread lookup which might be self."""
    tx_id = _create_pending_offer(tmp_db, amount_cents=200, message_id="offer-accept-1")
...
def test_accept_falls_back_to_thread_lookup(patch_agentmail, tmp_db):
...
def test_pay_sufficient_amount_unbans(patch_agentmail, tmp_db):
    """PAY with amount >= owed unbans the user."""
    xdb.ban_email(tmp_db, "baduser@test.com", "reversal", amount_owed_cents=300)
    assert xdb.is_banned(tmp_db, "baduser@test.com")

    xh.process_email(_pay_payload(300), db_path=tmp_db)

    assert not xdb.is_banned(tmp_db, "baduser@test.com")
    replies = patch_agentmail._replies
    assert len(replies) == 1
    body = _extract_json(replies[0]["text"])
...
def test_pay_insufficient_amount_stays_banned(patch_agentmail, tmp_db):
    """PAY with amount < owed keeps the user banned."""
    xdb.ban_email(tmp_db, "baduser@test.com", "reversal", amount_owed_cents=500)
    assert xdb.is_banned(tmp_db, "baduser@test.com")

    xh.process_email(_pay_payload(200), db_path=tmp_db)
...
def test_pay_overpayment_unbans(patch_agentmail, tmp_db):
    """PAY with amount > owed also unbans (overpayment is fine)."""
    xdb.ban_email(tmp_db, "baduser@test.com", "reversal", amount_owed_cents=200)

    xh.process_email(_pay_payload(500), db_path=tmp_db)
