...
from exchange.routes import (PROTOCOL_RE, handle_banned, handle_offer, handle_pay,
...
def process_email(payload: dict, db_path: str = DB_PATH) -> None:
    ...
    if ban_row and handle_banned(client, inbox_id, reply_to, from_addr, subject, text, db_path, ban_row, msg):
        return
    ...
    if msg_type and msg_type not in KNOWN_TYPES:
        _oops(client, inbox_id, reply_to, f"Unknown type: {msg_type}", {"code": "unknown_type",
              ...
              "spec": "https://june.kim/envelopay-spec.md"}, to=from_addr)
        return
    if msg_type == "WHICH" or stripped.upper() == "WHICH":
        handle_which(client, inbox_id, reply_to, from_addr, text, db_path)
        return
    if msg_type == "OFFER":
        handle_offer(client, inbox_id, reply_to, from_addr, text, db_path, message_id, thread_id, from_addr)
        return
    if msg_type == "PAY":
        handle_pay(client, inbox_id, reply_to, from_addr, text, db_path, message_id)
        return
    if msg_type:
        _oops(client, inbox_id, reply_to, "This exchange only handles WHICH and OFFER",
              {"code": "unsupported_flow", "sent": msg_type, "supported": ["WHICH", "OFFER"]}, to=from_addr)
        return
